package com.teavamc.leetcodes.NumUniqueEmails;
...
public class Solution {
    public int numUniqueEmails(String[] emails) {
        ...
        String pre,last,ableStr;
        for (String email:emails) {
            int p1 = email.indexOf("+");
            int p2 = email.indexOf("@");
            int l = email.length();
            if(p1<0){
                pre = email.substring(0,p2);
            }else {
                pre = email.substring(0,p1);
            }
            last = email.substring(p2,l);
            ableStr = pre.replace(".","")+last;
            ans.add(ableStr);
        }
        return ans.size();
    }
